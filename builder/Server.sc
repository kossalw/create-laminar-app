...
trait ServerModule extends AppScalaModule with DockerModule {
  def mainClass: T[Option[String]] = Some("server.MainServer")

  val serverDeps = zio ++ zioHttp ++ zioJson ++ tapirZioJson ++ pprintDep ++ caliban ++ calibanZio

  def ivyDeps = serverDeps

  object test extends ScalaTests with TestModule.ZioTest {
    def ivyDeps = serverDeps ++ zioTestDeps
  }
  ...
  object docker extends DockerConfig {
    ...
    def tags = List(s"repository/server:${gitCommit()}")

    def baseImage = "gcr.io/distroless/java21:latest"

    def labels = Map("version" -> gitCommit())

    // UDP ports the container will listen to
    def exposedUdpPorts = Seq.empty[Int]
